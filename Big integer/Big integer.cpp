...
inline long long  toLong(const Bint& b)
{
    return b.toLong();
}
inline Bint       Bpow(const Bint& b, int p)
{
    return b.Bpow(p);
}
inline size_t     numberOfDigits(const Bint& b)
{
    return b.numberOfDigits();
}
inline long long  sumOfDigits(const Bint& b)
{
    return b.sumOfDigits();
}
inline Bint       abs(const Bint& b)
{
    return b.abs();
}
inline Bint       reverse(const Bint& b)
{
    return b.reverse();
}
inline bool       isPalindrome(const Bint& b)
{
    return b.isPalindrome();
}
inline bool       isEven(const Bint& b)
{
    return b.isEven();
}
inline bool       isOdd(const Bint& b)
{
    return b.isOdd();
}
inline Bint       factorial(const Bint& b)
{
    return b.factorial();
}
inline bool       isPrime(const Bint& b)
{
    return b.isPrime();
}
...
static long long stringToLong(const string& n)
{
    if(lt(maxlong, n))
    {
        cerr << "Can't be expressed as long long: " << n << endl;
        exit(1);
    }
...
static string sub(const string& n1, const string& n2, bool ltn1n2)
{
    ...
    int i=0;
    while(res[ressize-1-i]=='0')
        ++i;
    if(i>0)
        res.erase(ressize-i,i);
...
Bint Bint::operator*(const Bint& b) const
{
    ...
    else // Use slow Bint
    {
        ...
        int bnsize = b.n.size();
        for(int i=1; i<bnsize; ++i)
            res.n = add(res.n, mul(n, b.n[i], i));
...
Bint Bint::operator/(const Bint& b) const
{
    ...
    else // Use slow Bint
    {
        ...
        for(int i = n.size()-b.n.size(); i >= 0; --i)
        {
            ...
            if(i>0)
                resi = resi * Bint(10) + Bint(n[i-1]-'0');
        }
...
bool Bint::operator<(const Bint& b) const
{
    if(s<b.s)
        return true;
    if(s==-1 && b.s==-1 && !lt(n, b.n) && n!=b.n)
        return true;
    if(s==1 && b.s==1 && lt(n, b.n))
        return true;
    return false;
}
...
Bint Bint::Bpow(int p) const
{
    if(p < 0)
    {
        cerr << "Negative power not implemented error: Bpow("<<*this<<","<<p<<")\n";
        exit(1);
    }
    if(p == 0)
    {
...
bool Bint::isPrime() const // All primes are of the form 6k-1 or 6k+1
{
    ...
    if(this->isEven())
        return false; // *this is multiple of 2
    if(this->sumOfDigits()%3 == 0)
        return false; // *this is multiple of 3
    if(n[0] == '5')
        return false; // *this is multiple of 5
    ...
    if(lt(this->n, maxlong)) // Use fast long long
    {
        ...
        {
            if(test%num==0)
                return false; // *this is multiple of num -> not prime
...
input:
1079461538614376927235889472194782343623820497009189813780932799382980310893089330891
7
output:
1079461538614376927235889472194782343623820497009189813780932799382980310893089330898
 */
